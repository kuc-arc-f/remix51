...
import type { TodoSchemaType } from './todo-validation';

export function TodoList() {
  const [todos, setTodos] = useState<TodoSchemaType[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingTodo, setEditingTodo] = useState<TodoSchemaType | undefined>();
  ...
  const handleAddTodo = (data: TodoSchemaType) => {
    setTodos([...todos, data]);
    setIsDialogOpen(false);
  };

  const handleEditTodo = (data: TodoSchemaType) => {
  ...
  const handleEdit = (todo: TodoSchemaType, index: number) => {
    setEditingTodo(todo);
